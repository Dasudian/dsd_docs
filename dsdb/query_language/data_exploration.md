...
* [The `SELECT` statement and the `WHERE` clause](/dsdb/query_language/data_exploration.md#the-select-statement-and-the-where-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The basic `SELECT` statement](/dsdb/query_language/data_exploration.md#the-basic-select-statement)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `SELECT` statement and arithmetic](/dsdb/query_language/data_exploration.md#the-select-statement-and-arithmetic)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `WHERE` clause](/dsdb/query_language/data_exploration.md#the-where-clause)
* [The `GROUP BY` clause](/dsdb/query_language/data_exploration.md#the-group-by-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The basic `GROUP BY` clause](/dsdb/query_language/data_exploration.md#the-basic-group-by-clause)   
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[The `GROUP BY` clause and `fill()`](/dsdb/query_language/data_exploration.md#the-group-by-clause-and-fill)  
* [The `INTO` clause](/dsdb/query_language/data_exploration.md#the-into-clause)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relocate data](/dsdb/query_language/data_exploration.md#relocate-data)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Downsample data](/dsdb/query_language/data_exploration.md#downsample-data)    

Limit and sort your results:

* [Limit query returns with `LIMIT` and `SLIMIT`](/dsdb/query_language/data_exploration.md#limit-query-returns-with-limit-and-slimit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit results per series with `LIMIT`](/dsdb/query_language/data_exploration.md#limit-the-number-of-results-returned-per-series-with-limit)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Limit the number of series returned with `SLIMIT`](/dsdb/query_language/data_exploration.md#limit-the-number-of-series-returned-with-slimit)  
* [Sort query returns with `ORDER BY time DESC`](/dsdb/query_language/data_exploration.md#sort-query-returns-with-order-by-time-desc)
* [Paginate query returns with `OFFSET`](/dsdb/query_language/data_exploration.md#paginate-query-returns-with-offset)

General tips on query syntax:

* [Multiple statements in queries](/dsdb/query_language/data_exploration.md#multiple-statements-in-queries)
* [Merge series in queries](/dsdb/query_language/data_exploration.md#merge-series-in-queries)
* [Time syntax in queries](/dsdb/query_language/data_exploration.md#time-syntax-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Relative time](/dsdb/query_language/data_exploration.md#relative-time)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Absolute time](/dsdb/query_language/data_exploration.md#absolute-time)
* [Regular expressions in queries](/dsdb/query_language/data_exploration.md#regular-expressions-in-queries)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and selecting measurements](/dsdb/query_language/data_exploration.md#regular-expressions-and-selecting-measurements)  
&nbsp;&nbsp;&nbsp;◦&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[Regular expressions and specifying tags](/dsdb/query_language/data_exploration.md#regular-expressions-and-specifying-tags)
...
The [series](/dsdb/concepts/glossary.md#series) are made up of the [measurement](/dsdb/concepts/glossary.md#measurement) `h2o_feet` and the [tag key](/dsdb/concepts/glossary.md#tag-key) `location` with the [tag values](/dsdb/concepts/glossary.md#tag-value) `santa_monica` and `coyote_creek`.
There are two [fields](/dsdb/concepts/glossary.md#field): `water_level` which stores floats and `level description` which stores strings.
...
Identifiers are database names, retention policy names, user names, measurement names, tag keys, and field keys.
 ...
 ```
* Fully qualify a measurement if you wish to query data from a different database or from a retention policy other than the default [retention policy](/dsdb/concepts/glossary.md#retention-policy).
...
Return data where the tag key `location` has no tag value (more on regular expressions [later](/dsdb/query_language/data_exploration.md#regular-expressions-in-queries)):
```sql
...
For more on `now()` and other ways to specify time in queries, see [time syntax in queries](/dsdb/query_language/data_exploration.md#time-syntax-in-queries).
...
See [Frequently Encountered Issues](/dsdb/troubleshooting/frequently_encountered_issues.md#understanding-the-time-intervals-returned-from-group-by-time-queries) for an explanation of the time intervals returned from `GROUP BY time()` queries.

Other things to note about `GROUP BY time()`:

* DSDBQL requires a `WHERE` clause if you're using `GROUP BY` with `time()`.
Note that unless you specify a different upper and lower bound for the time range, `GROUP BY` uses `epoch 0` as the lower bound and `now()` as the upper bound for the query - this can lead to [unexpected results](/dsdb/troubleshooting/frequently_encountered_issues.md#getting-unexpected-results-with-group-by-time).
...
If you want DSDB to automatically query and downsample all future data see [Continuous Queries](/dsdb/query_language/continuous_queries.md).
...
The `NOAA_water_database` database has two [series](/dsdb/concepts/glossary.md#series).
...
The [sample data](/dsdb/query_language/data_exploration.md#sample-data) need to be more intricate for the following sections.
...
```sql
> SELECT * FROM h2o_feet WHERE (location =~ /.*y.*/ OR location =~ /.*m.*.md) AND water_level > 0 LIMIT 4
```
or
<br>
<br>
...
See [the WHERE clause](/dsdb/query_language/data_exploration.md#the-where-clause) section for an example of how to return data where a tag key has a value and an example of how to return data where a tag key has no value using regular expressions.
